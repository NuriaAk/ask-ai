...
test.describe("Chat Functionality", () => {
  ...
  test("should send a message and receive a response in non-technical mode", async ({
    ...
    await expect(page.getByText(/GraphRAG response \(simple\)/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("should send a message and receive a response in technical mode", async ({
    ...
    // Wait for and verify assistant response (technical)
    await expect(page.getByText(/GraphRAG response \(technical\)/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("should not send empty messages", async ({ page }) => {
    ...
    // Send should remain disabled without input
  ...
  test("should get coordination-specific response", async ({ page }) => {
    ...
    await expect(page.getByText(/coordination patterns/i)).toBeVisible({
      timeout: 5000,
    });
  });
});
